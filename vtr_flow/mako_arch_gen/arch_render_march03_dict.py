...
def write_csv_file(filename, data):
    # Convert 1 to "on" and 0 to "off" for n2_lut5 and n1_lut6, and convert numeric values to integers if they are integers
    for fname, values in data.items():
        for key, value in values.items():
            if key in ['n2_lut5', 'n1_lut6', 'autolayout']:
...
def main():
    ...
    write_json_file(dir_name / 'arch_dict.json', arch_dict)
    # Write arch_dict to a new CSV file
    write_csv_file(dir_name / 'arch_dict.csv', arch_dict)
